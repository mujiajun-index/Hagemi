...
### 💻 本地运行：
    (直接双击项目内的`本地双击启动脚本.bat` 即可启动项目,默认端口为7860,可在脚本内修改。)
...
3.  在API Key中填入`PASSWORD`环境变量的值,如未设置则填入`123`
    或使用在⚙️系统设置添加的访问密钥:`sk-*`
    或使用聊天窗口发送消息`auth <you password>` 进行授权
...
*   新增IP黑名单:`BLACKLIST_IPS` 环境变量可设置黑名单IP，多个IP用逗号分隔，如：`127.0.0.1,192.168.1.1`。黑名单内的IP会自动拒绝访问。
...
4.  新增支持设置思考模型token，停用思考设置等
    * `gemini-2.5-pro-nothinking`：停用思考功能，回复内容更快。
    * `gemini-2.5-pro-thinking-32768`：最大思考模型token，最聪明，回复内容最详细。
    * `gemini-2.5-pro-thinking-*`：思考模型token，`*`，可根据需要设置。
    * `gemini-2.5-pro`：默认自动设置思考模型token，由模型自行决定。
5.  新增UI界面系统管理🔥
    * 新增管理系统设置功能，可在首页点击⚙️登录到系统设置界面，进行配置，无需重启项目即可生效。
    * 新增历史文件查看功能，可在界面中查看历史文件，支持查看占用情况和批量删除。
    * 新增代理路由映射功能，可在界面中设置代理路由映射，支持自定义路由代理转发。
    * 新增访问秘钥管理功能，可在界面中添加、删除、修改访问秘钥，支持设置次数限制，过期时间。
