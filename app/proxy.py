...
async def proxy_request(full_path: str, request: Request):
    ...
    matched_prefix = None
    rest_of_path = None
    for prefix, details in api_mappings.items():
        # 检查是否以前缀开头，需要考虑两种情况：
        # 1. 直接匹配前缀（如 /xai/v1...）
        ...
        if full_path.startswith(prefix) or full_path.startswith(prefix.lstrip('/')):
            if not details.get("enabled", False):
                return JSONResponse(content={"detail": f"Proxy for {prefix} is disabled."}, status_code=403)
    ...
    target_base_url = api_mappings[matched_prefix]["target"]
    ...
    logger.info(f"{datetimeStr}-[{client_ip}] Proxying request to {target_url}")
    # 转发请求
    try:
        # 复制请求头部，排除一些不必要的头部
